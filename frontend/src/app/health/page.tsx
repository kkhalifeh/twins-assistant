...
export default function HealthPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) => healthAPI.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['health'] })
      queryClient.invalidateQueries({ queryKey: ['vitals'] })
      setShowModal(false)
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h2 className="text-xl font-semibold mb-4">{editingLog ? 'Edit Health Data' : 'Log Health Data'}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex space-x-3">
                ...
                <button
                  type="submit"
                  disabled={createHealthLog.isPending || updateMutation.isPending}
                  className="flex-1 px-4 py-2 bg-primary-600 text-white rounded-md hover:bg-primary-700 disabled:opacity-50"
                >
                  {createHealthLog.isPending || updateMutation.isPending ? 'Saving...' : editingLog ? 'Update' : 'Save'}
                </button>
              </div>
            </form>
          </div>
