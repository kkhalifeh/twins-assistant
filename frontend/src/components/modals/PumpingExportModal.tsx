...
interface PumpingExportModalProps {
  onClose: () => void
}

export default function PumpingExportModal({ onClose }: PumpingExportModalProps) {
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/pumping/export/csv?startDate=${startDate}T00:00:00.000Z&endDate=${endDate}T23:59:59.999Z`,
        {
          headers: {
      ...
      a.href = url
      a.download = `pumping-export-${format(new Date(), 'yyyy-MM-dd')}.csv`
  ...
  const handleGenerateInsights = async () => {
    ...
    try {
      const token = localStorage.getItem('token')
      const response = await fetch(
        `/api/pumping/export/insights?startDate=${startDate}T00:00:00.000Z&endDate=${endDate}T23:59:59.999Z`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      const a = document.createElement('a')
      a.href = url
      a.download = `pumping-insights-${format(new Date(), 'yyyy-MM-dd')}.md`
      document.body.appendChild(a)
      a.click()
      window.URL.revokeObjectURL(url)
      document.body.removeChild(a)
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg w-full max-w-md">
        ...
        <div className="flex justify-between items-center p-6 pb-4 border-b">
          <h2 className="text-xl font-semibold">Export Pumping Report</h2>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="text-xs text-gray-500 space-y-1">
            <p><strong>CSV Export:</strong> Downloads all pumping data in spreadsheet format</p>
